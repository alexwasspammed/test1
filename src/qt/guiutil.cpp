...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("test1coin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("test1coin://"))
    {
        uri.replace(0, 11, "test1coin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Test1Coin.lnk";
}
...
{
    return GetAutostartDir() / "test1coin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Test1Coin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  test1coin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Test1Coin-Qt"));
